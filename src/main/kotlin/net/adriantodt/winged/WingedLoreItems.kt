...
object WingedLoreItems {
    val coreOfFlight = itemSettings()
        .rarity(Rarity.RARE).maxCount(1).loreItem(glint = true)
    val brokenCoreOfFlight = itemSettings()
        .maxCount(1).rarity(Rarity.UNCOMMON).loreItem()

    val batWing = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val blackFeather = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val angelFeather = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val phoenixFeather = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val friedChicken = itemSettings()
        .rarity(Rarity.UNCOMMON)
        .food { hunger(6).saturationModifier(1.6f).meat().snack() }.loreItem()
    val irrealityCrystal = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val demonicFlesh = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val bowlOfDyes = itemSettings().recipeRemainder(net.minecraft.item.Items.BOWL).loreItem()
    val attendeeTicket = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
    val vexEssence = itemSettings()
        .rarity(Rarity.UNCOMMON).loreItem()
