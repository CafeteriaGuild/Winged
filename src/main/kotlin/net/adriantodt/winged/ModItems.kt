...
val FRIED_CHICKEN = LoreItem(
    itemSettings().rarity(Rarity.UNCOMMON).food(
        FoodComponent.Builder().hunger(6).saturationModifier(0.6F).meat().snack().build()
    ), 2
)
...
val BAT_WING = LoreItem(itemSettings().rarity(Rarity.UNCOMMON), 2)
...
fun initItems() {
    ...
    identifier("bat_wing").item(BAT_WING)
