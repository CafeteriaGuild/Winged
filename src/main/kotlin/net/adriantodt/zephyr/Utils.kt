...
import net.adriantodt.zephyr.item.ZephyrItem
...
fun identifier(path: String) = Identifier("zephyr", path)
...
fun Identifier.item(block: Block) = apply {
    Registry.register(Registry.ITEM, this, BlockItem(block, Item.Settings().group(ZephyrItemGroups.main)))
}

operator fun String.rem(item: Item) {
    identifier(this).item(item)
}

fun itemSettings(): FabricItemSettings = FabricItemSettings().group(ZephyrItemGroups.main)

fun Item.Settings.food(block: FoodComponent.Builder.() -> Unit) = apply {
    food(FoodComponent.Builder().also(block).build())
}

fun Item.Settings.loreItem(amount: Int = 2, glint: Boolean = false) = ZephyrItem(this, amount, glint)
